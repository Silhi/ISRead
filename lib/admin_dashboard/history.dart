...
class HistoryDashboard extends StatefulWidget {
  ...
  @override
  HistoryDashboardState createState() => HistoryDashboardState();
}
...
class HistoryDashboardState extends State<HistoryDashboard> {
  ...
  void onSearch(String query) {
    setState(() {
      searchQuery = query;
      if (query.isEmpty) {
        filteredPinjam = pinjam; // Show all loans if search is empty
      } else {
        filteredPinjam = pinjam.where((loan) {
          final user = users.firstWhere(
            (u) => u.id == loan.id_user,
            orElse: () => UserModel(
              id: '1',
              username: 'Unknown',
              nrp: '000000', // Provide a default value for nrp
              email: 'unknown@example.com', // Provide a default value for email
              password: '',
              no_telpon: '',
              role: '',
              profpic: '',
  ...
  void onFilter(DateTime? date) {
    setState(() {
      selectedReturnDate = date;

      if (date == null) {
        filteredPinjam = pinjam; // Show all loans if no filter
      } else {
        filteredPinjam = pinjam.where((loan) {
          final returnRecord = kembali.firstWhere(
            (returnEntry) => returnEntry.id_pinjam == loan.id,
            orElse: () => PengembalianModel(
              id: '1',
              id_pinjam: loan.id,
              tgl_dikembalikan: 'Belum Kembali',
              bukti_foto: 'default_image.png',
            ),
          );

          DateTime returnDate =
              DateFormat('yyyy-MM-dd').parse(returnRecord.tgl_dikembalikan);
          return returnDate.isAtSameMomentAs(date) || returnDate.isAfter(date);
        }).toList();
      }
    });
  }
      ...
      child: Column(
        ...
        children: [
          ...
          TextButton.icon(
            ...
            label: Text(
              "Date: $formattedToday",
              style: TextStyle(color: const Color.fromARGB(255, 11, 15, 255)),
            ),
          ),
                  ...
                  child: DataTableTheme(
                    ...
                    child: DataTable(
                      ...
                      columns: [
                        ...
                        DataColumn(
                            label: const Text('Tanggal Pinjam',
                                style: TextStyle(fontWeight: FontWeight.bold))),
                        DataColumn(
                            label: const Text('Tanggal Kembali',
                                style: TextStyle(fontWeight: FontWeight.bold))),
                        DataColumn(
                            label: const Text('Denda',
                                style: TextStyle(fontWeight: FontWeight.bold))),
                        DataColumn(
                            label: const Text('Tanggal Pengembalian',
                                style: TextStyle(fontWeight: FontWeight.bold))),
                        DataColumn(
                            label: const Text('Action',
                                style: TextStyle(fontWeight: FontWeight.bold))),
                      ],
                      rows: filteredPinjam.map((loan) {
                        final user = users.firstWhere(
                          (u) => u.id == loan.id_user,
                          orElse: () => UserModel(
                            id: '1',
                            ...
                            nrp: '000000', // Provide a default value for nrp
                            email:
                                'unknown@example.com', // Provide a default value for email
                            password: '',
                            no_telpon: '',
                            role: '',
                            profpic: '',
                        ...
                        return DataRow(
                          cells: [
                            ...
                            DataCell(Text(loan.tgl_pinjam,
                                maxLines: 1, overflow: TextOverflow.ellipsis)),
                            DataCell(Text(loan.tgl_kembali,
                                maxLines: 1, overflow: TextOverflow.ellipsis)),
                            DataCell(Text(loan.denda,
                                maxLines: 1, overflow: TextOverflow.ellipsis)),
                            DataCell(Text(returnRecord.tgl_dikembalikan,
                                maxLines: 1, overflow: TextOverflow.ellipsis)),
                            DataCell(
                              Row(
                                mainAxisAlignment:
